...
void rsort_as_1 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm1;
__m128 xmm2;
__m128 xmm3;
    ...
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&startBlendMask[4*firstElements]));
    xmm0 = _mm_blendv_ps(xmm0, minusInf, blendMask);

    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm0 = _mm_blendv_ps(xmm0, plusInf, blendMask);
...
{ __m128 t = xmm2;
  ...
  xmm3 = _mm_max_ps(xmm3, t);
    ...
    mask = _mm_load_si128(reinterpret_cast<__m128i*>(&startMask[4*firstElements]));
    __m128i mask2 = _mm_load_si128(reinterpret_cast<__m128i*>(&endMask[4*lastElements]));
    
    mask2 = _mm_and_si128(mask, mask2);

    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm1), mask2, reinterpret_cast<char*>(first + 0*4));
}

void rsort_as_2 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm2;
__m128 xmm3;
    ...
    // Blend -Inf onto elements before our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&startBlendMask[4*firstElements]));
    xmm0 = _mm_blendv_ps(xmm0, minusInf, blendMask);

    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm1 = _mm_blendv_ps(xmm1, plusInf, blendMask);
   
    // Load first writeback mask in preparation for writeback
    mask = _mm_load_si128(reinterpret_cast<__m128i*>(&startMask[firstElements * 4]));
...
{ __m128 t = xmm1;
  ...
  xmm2 = _mm_max_ps(xmm2, t);
   ...
   _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm0), mask, reinterpret_cast<char*>(first + 0*4));
  
    // Load second writeback mask 
    mask =  _mm_load_si128(reinterpret_cast<__m128i*>(&endMask[lastElements * 4]));
   
   	xmm2 = _mm_movehl_ps(xmm2, xmm1);
   	xmm1 = _mm_shuffle_ps(xmm1, xmm2, 150);
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm1), mask, reinterpret_cast<char*>(first + 1*4));
}


void rsort_as_3 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm3;
__m128 xmm4;
__m128 xmm5;
__m128 xmm6;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm2 = _mm_blendv_ps(xmm2, plusInf, blendMask);
...
{ __m128 t = xmm5;
  ...
  xmm4 = _mm_max_ps(xmm4, t);
   ...
   _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm1), mask, reinterpret_cast<char*>(first + 0*4));
   ...
   _mm_store_ps(first + 1 * 4, xmm0);
   	
	xmm3 = _mm_movelh_ps(xmm3, xmm5);
   	xmm2 = _mm_movelh_ps(xmm2, xmm4);
   	xmm3 = _mm_shuffle_ps(xmm3, xmm2, 61); 
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm3), mask, reinterpret_cast<char*>(first + 2*4));
}

void rsort_as_4 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm3 = _mm_load_ps(first + 3 * 4);
__m128 xmm4;
__m128 xmm5;
__m128 xmm6;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm3 = _mm_blendv_ps(xmm3, plusInf, blendMask);
...
{ __m128 t = xmm5;
  ...
  xmm4 = _mm_max_ps(xmm4, t);
   ...
   _mm_store_ps(first + 2 * 4, xmm3);
   	xmm5 = _mm_movehl_ps(xmm5, xmm2);
   	xmm5 = _mm_shuffle_ps(xmm5, xmm2, 45);
   
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm5), mask, reinterpret_cast<char*>(first + 3*4));
}

void rsort_as_5 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm3 = _mm_load_ps(first + 3 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm5;
...
__m128 xmm7;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm4 = _mm_blendv_ps(xmm4, plusInf, blendMask);
...
{ __m128 t = xmm7;
  ...
  xmm3 = _mm_max_ps(xmm3, t);
   ...
   _mm_store_ps(first + 2 * 4, xmm2);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm5, 123);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm0, 114);
   _mm_store_ps(first + 3 * 4, xmm0);
   	xmm5 = _mm_movelh_ps(xmm5, xmm7);
   	xmm3 = _mm_shuffle_ps(xmm3, xmm4, 48);
   	xmm5 = _mm_shuffle_ps(xmm5, xmm3, 140);
   
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm5), mask, reinterpret_cast<char*>(first + 4*4));
}

void rsort_as_6 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm3 = _mm_load_ps(first + 3 * 4);
__m128 xmm6;
__m128 xmm7;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm5 = _mm_blendv_ps(xmm5, plusInf, blendMask);
...
{ __m128 t = xmm7;
  ...
  xmm6 = _mm_max_ps(xmm6, t);
   ...
   _mm_store_ps(first + 1 * 4, xmm5);
   	xmm0 = _mm_movehl_ps(xmm0, xmm2);
   	xmm1 = _mm_movelh_ps(xmm1, xmm0);
   	xmm1 = _mm_movehl_ps(xmm1, xmm3);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm1, 150);
   _mm_store_ps(first + 2 * 4, xmm0);
   	xmm2 = _mm_shuffle_ps(xmm2, xmm3, 97);
   	xmm2 = _mm_shuffle_ps(xmm2, xmm2, 114);
   _mm_store_ps(first + 3 * 4, xmm2);
   	xmm3 = _mm_shuffle_ps(xmm3, xmm7, 32);
   	xmm4 = _mm_movelh_ps(xmm4, xmm6);
   	xmm6 = _mm_movehl_ps(xmm6, xmm3);
   	xmm3 = _mm_shuffle_ps(xmm3, xmm6, 104);
   _mm_store_ps(first + 4 * 4, xmm3);
   	
	xmm7 = _mm_unpacklo_ps(xmm7, xmm4);
   	xmm4 = _mm_unpackhi_ps(xmm4, xmm7);
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm4), mask, reinterpret_cast<char*>(first + 5*4));
}

void rsort_as_7 (float* first, int firstElements, int lastElements) {
...
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
__m128 xmm3 = _mm_load_ps(first + 3 * 4);
__m128 xmm7;
__m128 xmm8;
__m128 xmm9;
__m128 xmm10;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm6 = _mm_blendv_ps(xmm6, plusInf, blendMask);
...
{ __m128 t = xmm9;
  ...
  xmm8 = _mm_max_ps(xmm8, t);
   ...
   _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm3), mask, reinterpret_cast<char*>(first + 0*4));
...
// Load second writeback mask
   ...
   _mm_store_ps(first + 4 * 4, xmm4);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm7, 48);
   	xmm0 = _mm_unpackhi_ps(xmm0, xmm6);
   _mm_store_ps(first + 5 * 4, xmm0);
   	
	xmm7 = _mm_movelh_ps(xmm7, xmm9);
   	xmm6 = _mm_movelh_ps(xmm6, xmm8);
   	xmm7 = _mm_shuffle_ps(xmm7, xmm6, 61);
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm7), mask, reinterpret_cast<char*>(first + 6*4));
}

void rsort_as_8 (float* first, int firstElements, int lastElements) {
...
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
__m128 xmm3 = _mm_load_ps(first + 3 * 4);
__m128 xmm7 = _mm_load_ps(first + 7 * 4);
__m128 xmm8;
__m128 xmm9;
__m128 xmm10;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm7 = _mm_blendv_ps(xmm7, plusInf, blendMask);
...
// Load second writeback mask
   ...
   _mm_store_ps(first + 4 * 4, xmm1);
   	xmm4 = _mm_shuffle_ps(xmm4, xmm0, 19);
   	xmm0 = _mm_movehl_ps(xmm0, xmm3);
   	xmm3 = _mm_movehl_ps(xmm3, xmm4);
   	xmm4 = _mm_shuffle_ps(xmm4, xmm3, 120);
   _mm_store_ps(first + 5 * 4, xmm4);
   	xmm9 = _mm_shuffle_ps(xmm9, xmm0, 35);
   	xmm9 = _mm_shuffle_ps(xmm9, xmm9, 39);
   _mm_store_ps(first + 6 * 4, xmm9);
 
	xmm6 = _mm_shuffle_ps(xmm6, xmm10, 49);
   	xmm0 = _mm_movehl_ps(xmm0, xmm7);
   	xmm6 = _mm_shuffle_ps(xmm6, xmm0, 216);
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm6), mask, reinterpret_cast<char*>(first + 7*4));
}

void rsort_as_9 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm8 = _mm_load_ps(first + 8 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
...
__m128 xmm12;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm8 = _mm_blendv_ps(xmm8, plusInf, blendMask);
...
{ __m128 t = xmm9;
  ...
  xmm7 = _mm_max_ps(xmm7, t);
   ...
   _mm_store_ps(first + 6 * 4, xmm7);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm5, 18);
   	xmm5 = _mm_movehl_ps(xmm5, xmm8);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm5, 34);
   _mm_store_ps(first + 7 * 4, xmm0);
   	
	xmm9 = _mm_shuffle_ps(xmm9, xmm10, 48);
   	xmm8 = _mm_shuffle_ps(xmm8, xmm11, 48);
   	xmm9 = _mm_shuffle_ps(xmm9, xmm8, 34);
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm9), mask, reinterpret_cast<char*>(first + 8*4));
}

void rsort_as_10 (float* first, int firstElements, int lastElements) {

__m128 xmm0 = _mm_load_ps(first + 0 * 4);
__m128 xmm8 = _mm_load_ps(first + 8 * 4);
__m128 xmm1 = _mm_load_ps(first + 1 * 4);
__m128 xmm9 = _mm_load_ps(first + 9 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
...
__m128 xmm11;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm9 = _mm_blendv_ps(xmm9, plusInf, blendMask);
...
{ __m128 t = xmm11;
  ...
  xmm10 = _mm_max_ps(xmm10, t);
   ...
   _mm_store_ps(first + 8 * 4, xmm9);
   	
	xmm11 = _mm_shuffle_ps(xmm11, xmm8, 33);
   	xmm11 = _mm_shuffle_ps(xmm11, xmm8, 50);
   _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm11), mask, reinterpret_cast<char*>(first + 9*4));
}

void rsort_as_11 (float* first, int firstElements, int lastElements) {
...
__m128 xmm9 = _mm_load_ps(first + 9 * 4);
__m128 xmm2 = _mm_load_ps(first + 2 * 4);
__m128 xmm10 = _mm_load_ps(first + 10 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
__m128 xmm7 = _mm_load_ps(first + 7 * 4);
...
__m128 xmm14;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm10 = _mm_blendv_ps(xmm10, plusInf, blendMask);
...
{ __m128 t = xmm13;
  ...
  xmm11 = _mm_max_ps(xmm11, t);
    ...
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm9), mask, reinterpret_cast<char*>(first + 0*4));
   ...
   _mm_store_ps(first + 8 * 4, xmm6);
   	xmm0 = _mm_shuffle_ps(xmm0, xmm8, 144);
   	xmm0 = _mm_unpackhi_ps(xmm0, xmm10);
   _mm_store_ps(first + 9 * 4, xmm0);
   	
	xmm8 = _mm_movelh_ps(xmm8, xmm13);
   	xmm10 = _mm_movelh_ps(xmm10, xmm11);
   	xmm8 = _mm_shuffle_ps(xmm8, xmm10, 40);
    _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm8), mask, reinterpret_cast<char*>(first + 10*4));
}

void rsort_as_12 (float* first, int firstElements, int lastElements) {
...
__m128 xmm10 = _mm_load_ps(first + 10 * 4);
__m128 xmm3 = _mm_load_ps(first + 3 * 4);
__m128 xmm11 = _mm_load_ps(first + 11 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
__m128 xmm7 = _mm_load_ps(first + 7 * 4);
...
__m128 xmm13;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm11 = _mm_blendv_ps(xmm11, plusInf, blendMask);
...
__m128 xmm11 = _mm_load_ps(first + 11 * 4);
__m128 xmm4 = _mm_load_ps(first + 4 * 4);
__m128 xmm12 = _mm_load_ps(first + 12 * 4);
__m128 xmm5 = _mm_load_ps(first + 5 * 4);
__m128 xmm6 = _mm_load_ps(first + 6 * 4);
__m128 xmm7 = _mm_load_ps(first + 7 * 4);
__m128 xmm13;
...
__m128 xmm16;
    ...
    // Blend +Inf onto elements after our starting point
    blendMask = _mm_load_ps(reinterpret_cast<float*>(&endBlendMask[4*lastElements]));
    xmm12 = _mm_blendv_ps(xmm12, plusInf, blendMask);
...
{ __m128 t = xmm15;
  ...
  xmm13 = _mm_max_ps(xmm13, t);
   ...
   _mm_maskmoveu_si128(reinterpret_cast<__m128i>(xmm11), mask, reinterpret_cast<char*>(first + 0*4));
